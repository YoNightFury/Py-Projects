...
def updateAndDisplayMatrix(matrix):
    os.system('cls')
    displayCells(matrix)
    time.sleep(1)
    ...
    for rowIndex,row in enumerate(matrix):      #gives row index and list of elements in row
        for columnIndex,cell in enumerate(row):   #gives individual elements of each row with its column index
            ...
            if (cell==' ' and count==3):
                tempMatrix[rowIndex][columnIndex]='#'
                alive+=1
                os.system('cls')
                displayCells(tempMatrix)
                time.sleep(1)
            elif cell==' ':
                tempMatrix[rowIndex][columnIndex]=' '
...
h=4 #height or no. of rows of dead-alive matrix
w=4 #width or no. of columns of dead-alive matrix
matrix=generateMatrix(h, w)

while True:
        
        matrix=updateAndDisplayMatrix(matrix)
        time.sleep(1)
        if matrix==None:
            break
